...
class PicocliCommandLineParser {
    ...
    private static Command getCommandForParseResult(CommandOrchestratorExecution commandExecution, ParseResult parseResult) {
        String commandName = parseResult.commandSpec().name();
        Command command = commandExecution.getRootCommand();
    ...
    public static void parse(CommandLine picocliCommandLine, CommandOrchestratorExecution commandExecution) {
        try {
            ...
            while (!commandLineQueue.isEmpty()) {
                ...
                if (currentParseResult.expandedArgs().isEmpty()) {
                    if (currentCommand.getExecutesWithoutArguments()) {
                        commandExecution.setCalledCommand(currentCommand);
                    }
                    commandExecution.setExitCode(usageHelp(currentCommandLine));
                    return;
                }
            ...
            if (!nonInteractiveOptionProvided) {
                ...
                if (response.equalsIgnoreCase("yes") || response.equalsIgnoreCase("y")) {
                    ...
                    parse(executedCommandLine, commandExecution);
                    return;
                }
            }
            executedCommandLine.getErr().println(unmatchedParameterMessage);
